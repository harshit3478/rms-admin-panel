...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    if (!id) return;
    getDoc(doc(db, 'blog', id)).then(snap => {
      if (snap.exists()) {
        const d = snap.data();
        setForm({
          heading: d.heading ?? '',
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Edit blog post</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Field label="Body *">
          <textarea value={form.body} onChange={e => set('body', e.target.value)} rows={8} className={`${inputCls} resize-y`} />
        </Field>

        <Field label="Image URL" hint="Direct link to a cover image">
        ...
        <Field label="Citation">
          <input value={form.citation} onChange={e => set('citation', e.target.value)} className={inputCls} />
        </Field>
